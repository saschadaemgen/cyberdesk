...
(typing, backspace, focus, caret, paste, Enter) while keeping the
host-capture guarantee that no character reaches the renderer; where a
capability is genuinely impossible under host capture it is stated plainly
rather than faked. The weak-password warning and its override button appear
only when the password is actually weak, with empty as its own neutral
state, so no two indicators ever contradict one another (a status display
must never lie). Em dashes are forbidden in every user-facing surface and
repo document, recorded as a binding rule in CLAUDE.md and enforced by an
automatic pre-push guard, because copy that instantly reads as
machine-written undermines a product built on trust. Settings become a
full-screen layer: the gear opens it over the full width and height, the
workspace slides away with an animation against a fixed background,
sectioned and built to grow, correct from 1920x1080 up to roughly 5200
wide, with no scrollbar crutch and no clipped controls. A declinable passkey
prompt follows master-password setup, and the application ships a real icon.

*Findings and root causes, isolated before fixing (Stage A).*

1. **The entry was not unusable by accident: Esc was destructive.** The lock
   page bound Esc to `cancel_capture`, which on the mandatory first-launch
   setup silently threw the typed password away and restarted the flow, with
   no visible focus state to explain what had happened. Fixed with
   `vault::key_escape`: with text it clears the ENTRY only; on an empty
   confirm step it goes back exactly one step; the mandatory flows (setup,
   unlock) never abort. The entry now shows its live focus, carries a
   neutral placeholder, and each step is titled ("First launch, step 1 of
   2") with a key legend that matches the real semantics.

2. **The weak warning ignored the verdict because of a CSS precedence bug.**
   `.weak` / `.vweak` set `display: flex` on the class, which overrides the
   user-agent `[hidden] { display: none }` rule, so `el.hidden = true` had no
   visual effect and the warning plus its override button were painted
   permanently, next to a meter reading "Very strong". Root-caused rather
   than patched around: both page stylesheets now carry
   `[hidden] { display: none !important }`, and the weak block additionally
   keys on (meter shown AND `weak_pending`). Same class of bug was checked
   for on every other page (`hud`, `command`, `onion`, `mfzone`), which
   already declared their `[hidden]` overrides correctly.

3. **`NotSupportedError` on Add passkey was not an FFI defect.** A probe on
   the target machine shows `webauthn.dll` at API v9 but
   `WebAuthNIsUserVerifyingPlatformAuthenticatorAvailable` returning FALSE:
   no Windows Hello PIN, fingerprint or face is enrolled, so a
   platform-attachment MakeCredential cannot succeed. The algorithm list
   (ES256 first, RS256 fallback), the hmac-secret extension shape and the
   options struct versions were re-checked against the pinned bindings and
   are correct. Enrollment now pre-checks `hello_ready()` and refuses with
   the actual next step; the settings row shows that state instead of
   offering a dead button and re-checks itself so it lights up once Hello is
   configured; every surviving API error is translated to plain language
   (NotSupported, InvalidState, Constraint, cancelled) rather than surfaced
   as a raw W3C name.

*The em dash rule, precisely.* U+2014, U+2013 and their HTML entity
spellings are forbidden repo-wide, including source comments, not only in UI
strings: the repo is public open core, so comments are read copy too, and a
rule with a "comments do not count" hole decays. Enforcement is two-layered:
`scripts/no-emdash.mjs` scans the **staged tree** (the right unit for a
pre-push guard: an unrelated uncommitted WIP cannot block a push, and
whoever stages it is flagged then), and a `cargo test` tripwire
(`no_em_dashes_in_ui_and_docs`) covers the product copy and living docs
...
iron law or the CD-42 unlock model.
